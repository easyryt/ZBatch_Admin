...
import CreateTestModal from "./CreateBatchTestModal";
...
const BatchTest = () => {
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(
        `https://www.backend.zbatch.in/admin/batches/test/subjects/tests/delete/${selectedSelectedId}`,
        {
          headers: {
            "x-admin-token": token,
          },
  ...
  const fetchTests = async () => {
    try {
      const response = await axios.get(
        `https://www.backend.zbatch.in/admin/batches/test/subjects/tests/getAll/${subjectId}`,
        {
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box>
          <IconButton title="Edit" onClick={()=>navigate(`/dashboard/batch-question-details/${batchId}/${params.row._id}`)}>
            <VisibilityIcon />
          </IconButton>
        </Box>
      ),
