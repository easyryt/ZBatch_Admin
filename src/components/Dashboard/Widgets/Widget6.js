...
const Widget6 = () => {
  ...
  const [testRevenue, setTestRevenue] = useState(null); // State to store batch revenue data
  const [loading, setLoading] = useState(true); // Loading state for API call
  const [error, setError] = useState(null); // Error state for API call
  const navigate = useNavigate(); // Initialize navigate

  useEffect(() => {
    const fetchTestRevenue = async () => {
      try {
        const token = Cookies.get("token"); // Get the token from cookies
        const response = await axios.get(
          "http://www.backend.zbatch.in/admin/dashBoard/totalDirectTestRevenue",
          {
            headers: {
        ...
        setTestRevenue(response.data.data); // Set batch revenue
    ...
    const fetchBatchRevenue = async () => {
      try {
        const token = Cookies.get("token"); // Get the token from cookies
        const response = await axios.get(
          "http://www.backend.zbatch.in/admin/dashBoard/totalBatchRevenue",
          {
            headers: {
              "Content-Type": "application/json",
              "x-admin-token": token, // Pass token in header
  ...
  const overallRevenue =
    (batchRevenue !== null ? batchRevenue : 0) +
    (testRevenue !== null ? testRevenue : 0);
      ...
      {loading ? (
        ...
        >
          <span style={{ color: "#1A237E", fontWeight: "bold" }}>
            ₹{overallRevenue !== null ? overallRevenue.toLocaleString() : 0}
          </span>{" "}
          Overall Revenue
        </Typography>
      )}
